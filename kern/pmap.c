...
boot_alloc(uint32_t n)
{
	...
	int page_num = n / PGSIZE + ((n % PGSIZE)?1:0);
	
	char* ret = nextfree;
	nextfree += PGSIZE*page_num;
	return ret;
}
...
page_init(void)
{
	...
	uint32_t free_addr = (uint32_t)PADDR(boot_alloc(0));
	for(uint32_t i = EXTPHYSMEM/PGSIZE;i <  free_addr/PGSIZE;i++){
		pages[i].pp_ref = 1;
	}

	// except allocated memory
	for(uint32_t i =free_addr/PGSIZE;i < npages;i++ ){
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref !=0 || pp->pp_link != NULL) panic("free page error kva : %x",page2kva(pp));
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	int pdx = PDX(va);
	int ptx = PTX(va);
	if(pgdir[pdx] & PTE_P) return ((pte_t *)KADDR(PTE_ADDR(pgdir[pdx]))) + ptx;
	if(create){
		struct PageInfo* page = page_alloc(ALLOC_ZERO);
		if(page){
			page->pp_ref++;
			pgdir[pdx] = page2pa(page) | PTE_P | PTE_W | PTE_U;
			pte_t * pte = page2kva(page);
			return pte + ptx;
		}
	}
	return NULL;
